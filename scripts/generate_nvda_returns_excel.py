...
def main():
    ...
    with pd.ExcelWriter(OUTPUT_PATH, engine="xlsxwriter", datetime_format="yyyy-mm-dd") as writer:
        ...
        # Sheet: Returns (formula-based)
        ws_ret = wb.add_worksheet("Returns")
        ws_ret.write(0, 0, "Date", bold)
        ws_ret.write(0, 1, "Return (Simple %)", bold)

        # Excel rows: header at row 1 -> first data at row 2
        n_rows = len(df)
        ret_row = 1
        for i in range(1, n_rows):
            data_row_excel = i + 1  # df index -> Excel row
            ws_ret.write_datetime(ret_row, 0, df.loc[i, "Date"].to_pydatetime(), date_fmt)
            # Return_t = Close_t / Close_{t-1} - 1, referencing Data!E (Close)
        ...
        # Sheet: Summary (observations and conclusion)
